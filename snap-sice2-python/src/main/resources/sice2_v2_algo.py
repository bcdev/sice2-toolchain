...
class Sice2V2Algo:
    ...
    def view_geometry(self, olci_scene):
    ...
    def rinff(self, cos_sza, cos_vza, theta):
    ...
    def prepare_processing(self, olci_scene, angles):
        ...
        psi = self.rinff(angles["cos_sza"], angles["cos_vza"], angles["theta"])
    ...
    def snow_properties(self, olci_scene, angles, snow):
    ...
    def aerosol_properties(self, height, cos_sa, aot=0.07):
    ...
    def prepare_coef(self, aerosol, angles):
        # args = (
        #     aerosol.tau,
        #     aerosol.g,
        #     aerosol.p,
        ...
        t1t2, albatm, r = self.prepare_coef_xarray(
            aerosol.tau,
            aerosol.g,
            aerosol.p,
            angles.cos_sza,
    ...
    def snow_albedo_solved(self, olci_scene, angles, aerosol, atmosphere, snow):
    ...
    def snow_albedo_direct(self, olci_scene, angles, aerosol, atmosphere, snow, impurities):
    ...
    def compute_BBA(self, olci_scene, snow, angles, compute_polluted=True):
        ...
        if compute_polluted:
            ...
            _, _, snow.rp3[iind_pol] = self.BBA_calc_pol(
                snow.rp[iind_pol].values.T, asol, sol_vis, sol_nir, sol_sw
            )
            _, _, snow.rs3[iind_pol] = self.BBA_calc_pol(
    ...
    def funp(self, x, al, sph_calc, u1):
    ...
    def BBA_calc_clean(self, al, u1, mode='spherical'):
        # CalCULATION OF BBA of clean snow
        # Original method: Recalculating spectrum and integrating it.
        # This is the exact, but slow mehtod to calculate clean snow BBA.
        # For each clean pixel, the derived spectrum is caluclated from u1, al and
        ...
        def func_integ(x):
            return self.funp(x, al, sph_calc, u1)

        # visible(0.33-0.7micron)
        flux_vis = self.qsimp(func_integ, 0.33, 0.7)

        # near-infrared (0.7-2.4micron)
        flux_nir = self.qsimp(func_integ, 0.7, 2.4)
    ...
    def BBA_calc_pol(self, alb, asol, sol_vis, sol_nir, sol_sw):
        ...
        _, a1, b1, c1 = self.quad_func(alam2, alam3, alam5, r2, r3, r5)
        coef1, coef2 = self.analyt_func(0.33, 0.7)
        ajx1 = a1 * sol_vis
        ajx2 = b1 * coef1
        ajx3 = c1 * coef2
        ...
        _, a2, b2, c2 = self.quad_func(alam5, alam6, alam7, r5, r6, r7)
        coef3, coef4 = self.analyt_func(0.7, 0.865)
        ajx1 = a2 * asol
        ajx2 = b2 * coef3
        ajx3 = c2 * coef4
        aj2 = ajx1 + ajx2 + ajx3
    ...
    def process(self, olci_scene, compute_polluted=True, **kwargs):
        angles = self.view_geometry(olci_scene)
        olci_scene, snow = self.prepare_processing(olci_scene, angles)
        olci_scene, angles, snow = self.snow_properties(olci_scene, angles, snow)
        aerosol = self.aerosol_properties(olci_scene.elevation, angles.cos_sa, aot=0.07)
        atmosphere = self.prepare_coef(aerosol, angles)

        # first guess for the snow spherical albedo
        olci_scene, snow = self.snow_albedo_solved(olci_scene, angles, aerosol, atmosphere, snow)

        # retrieving snow impurities
        impurities = self.snow_impurities(snow)

        olci_scene, snow = self.snow_albedo_direct(
            olci_scene, angles, aerosol, atmosphere, snow, impurities
        )

        snow = self.compute_BBA(olci_scene, snow, angles, compute_polluted=compute_polluted)
        return snow

    def process_by_chunk(self, olci_scene, chunk_size=150000, compute_polluted=True):

# -*- coding: utf-8 -*-
# ----------------------------------------------------------------------------
# Created By  : Baptiste Vandecrux (GEUS), Olaf Danne (BC)
# Created Date: date/month/time ..etc
# version ='1.0'
# ---------------------------------------------------------------------------
"""
Script containing functions implementing pySICEv2 algorithm.
For usage within a snappy operator.

@authors: Baptiste Vandecrux (GEUS), Olaf Danne (BC)
"""
# ---------------------------------------------------------------------------
import os
...
from sice2_v2_constants import thv0, sol_vis, sol_nir, sol_sw, asol
from sice2_v2_constants import wls, bai, xa, ya, f0, f1, f2, bet, gam
...
def ozone_scattering(ozone, tozon, sza, vza, toa):
    """
    Provides toa corrected for ozone.
...
def view_geometry(olci_scene):
    ...
    :param olci_scene: OLCI scene with all bands read in SiceV2Io.open_tif
    :return: xr.Dataset() with angles
    """
...
def rinff(cos_sza, cos_vza, theta):
    """
    This is the "theoretical" value for R0, the reflectance of a semi-infinite non-absorbing snow layer.
...
def prepare_processing(olci_scene, angles):
    """
    Prepares processing (i.e. filters pixels unsuitable for retrieval).

    :param olci_scene: OLCI scene with all bands read in SiceV2Io.open_tif.
    :param angles: geometry
    :return: modified olci_scene, xr.Dataset() with snow filter
    """
...
def snow_properties(olci_scene, angles, snow):
    ...
    influenced by atmospheric scattering and absorption processes) imaginary part of the ice refractive
    index at 865 and 1020nm.

    :param olci_scene:
    :param angles:
    :param snow:
    :return: modified olci_scene, xr.Dataset() with angles, xr.Dataset() with snow filter
    """
...
def aerosol_properties(height, cos_sa, aot=0.07):
    """
    Provides various aerosol properties.

    :param height:
    :param cos_sa:
    :param aot:
    :return: xr.Dataset() with aerosol properties
    """
...
def prepare_coef(aerosol, angles):
    """
    Provides atmosphere coefficients.

    :param aerosol: xr.Dataset() with aerosol properties
    :param angles: xr.Dataset() with angles
    :return: xr.Dataset() with atmosphere coefficients.
    """
...
def prepare_coef_xarray(tau, g, p, cos_sza, cos_vza, inv_cos_za, gaer, taumol, tauaer):
    """
    Computation of atmosphere coefficients using xarray.
...
def snow_albedo_solved(olci_scene, angles, aerosol, atmosphere, snow):
    """
    Solving iteratively the transcendental equation. Update 2022: for all pixels!
...
# NOTE: option 'cache=True' does not work with snappy (OD, 20221012)
# @numba.jit(nopython=True, cache=True)
@numba.jit(nopython=True)
def f(albedo, t1t2, r0, u1, u2, albatm, r, toa):
    """
    todo: check ATBD for this function
...
def snow_impurities(snow):
    """
    Analysis of snow impurities (the concentrations below 0.0001 are not reliable).

    :param snow:
    :return: xr.Dataset() with impurities.
    """
...
def snow_albedo_direct(olci_scene, angles, snow, impurities):
    """
...
def funp(x, al, sph_calc, u1):
    ...
    Original way to recalculate the spectral albedo from al, u1 and the ice refraction index.
    Uses numpy interpolation.
    Currently not used. But do not remove.

    :param x: input wavelength (should work with any)
    :param al: absorption length
    :param sph_calc: sph_calc= 0 for planar =1 for spherical
    :param u1:
    :return: f1 * solar_flux
    """
...
def qsimp(func, a, b):
    ...
    for j in range(jmax):
        if j == 0:
            ...
            this_sum = 0.0
            for jj in range(it):
                this_sum = this_sum + func(x)
                x = x + delta
            st = 0.5 * (st + (b - a) * this_sum / tnm)
        s = (4.0 * st - ost) / 3.0
        if j > 4:
            if np.abs(s - this_os) < eps * np.abs(this_os):
                return s
            if (s == 0) and (this_os == 0.0):
                return s
        this_os = s
        ost = st
...
# @numba.jit(nopython=True, cache=True)
@numba.jit(nopython=True)
def bba_calc_pol(alb, asolar, solar_vis, solar_nir, solar_sw):
    """
    polluted snow
    ...
    bba_vis = aj1 / solar_vis
    bba_nir = (aj2 + aj3) / solar_nir  # here segment 2.1 and 2.2 are summed
    bba_sw = (aj1 + aj2 + aj3) / solar_sw
...
def process(olci_scene, compute_polluted=True):
    """
    Does the processing without chunking.

    :param olci_scene: OLCI scene with all bands read in SiceV2Io.open_tif
    :param compute_polluted: boolean
    :return: xr.Dataset() with snow parameters
    """
    ...
    olci_scene, snow = snow_albedo_direct(olci_scene, angles, snow, impurities)

    snow = compute_bba(olci_scene, snow, angles, compute_polluted=compute_polluted)
    return snow


def process_by_chunk(olci_scene, chunk_size=150000):
    """
    Does the processing with chunking.

    :param olci_scene: OLCI scene with all bands read in SiceV2Io.open_tif
    :param chunk_size: xarray chunk size
    :return: xr.Dataset() with snow parameters
    """
